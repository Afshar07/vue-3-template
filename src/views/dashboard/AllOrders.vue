            ...
            <tr v-for="(item,idx) in orders.ordersList" :key="idx">
              ...
              <td>
                ...
                <div v-if="item['status'] ===4" class="tooltip" data-tip="Ended">
                  <CheckIcon class="w-8 h-8 fill-green-500"></CheckIcon>
                </div>
                <div v-if="item['status'] ===2" class="tooltip" data-tip="Canceled">
                  <CancelIcon class="w-8 h-8 fill-red-500"></CancelIcon>
                </div>
                <div v-if="item['status'] ===3" class="tooltip" data-tip="Paused">
              ...
              <td>
                <button @click="selectedOrder = item" type="button"
                        class="btn btn-sm text-white border-none cursor-pointer  bg-primary">
...
let selectedOrder = ref<any>(null)
...
async function getAllOrders() {
  try {
    appStore.showOverlay = true
    const res = await api.getAllOrders.setParams({

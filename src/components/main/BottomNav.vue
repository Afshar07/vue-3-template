<template>
  ...
  <div
      class="w-full flex relative items-center justify-around dark:text-white h-16 bg-white dark:bg-dark-muted shadow-xl border-t border-t-gray-200 dark:border-t-gray-800">
    <transition name="slide-up" appear>
      <div  class="absolute bottom-full    w-full" @click="closeBottomModal"
            v-if="isRenderingBottomModal">
        <div class="fixed top-0 right-0 w-full h-full  bg-gray-500/70 opacity-50" @click="closeBottomModal" ></div>
        <div @click.prevent class="w-full z-50  relative p-5 rounded-t-xl bg-slate-50">
          <div class="flex border-dotted  border-b  w-full pb-3 items-center gap-2">
            <DynamicIcon :key="tmpSubItems.icon"  :icon="tmpSubItems.icon"></DynamicIcon>
            <strong class="text-gray-900">{{ tmpSubItems.title }}</strong>
          </div>
          <div class="w-full flex  flex-col items-start">
            <router-link v-for="(item,idx) in tmpSubItems.children" :key="idx" :to="item['link']" class="flex  items-center text-gray-500 fill-gray-500 p-3 gap-2">
...
const
    hasAccess = function (accessRoles) {
